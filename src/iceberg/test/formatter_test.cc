...
TEST(FormatterTest, VectorFormat) {
  ...
  EXPECT_EQ("[]", compat::format("{}", empty));

  std::vector<int> nums = {1, 2, 3, 4, 5};
  EXPECT_EQ("[1, 2, 3, 4, 5]", compat::format("{}", nums));

  std::vector<std::string> names = {"Alice", "Bob", "Charlie"};
  EXPECT_EQ("[Alice, Bob, Charlie]", compat::format("{}", names));
}
...
TEST(FormatterTest, MapFormat) {
  ...
  EXPECT_EQ("{Alice: 30, Bob: 25, Charlie: 35}", compat::format("{}", ages));
}
...
TEST(FormatterTest, UnorderedMapFormat) {
  ...
  std::string str = compat::format("{}", scores);
...
TEST(FormatterTest, NestedContainersFormat) {
  ...
  EXPECT_EQ("[{a: 1, b: 2}, {c: 3, d: 4}]", compat::format("{}", nested));

  std::map<std::string, std::vector<int>> nested_map = {
      {"primes", {2, 3, 5, 7, 11}}, {"fibonacci", {1, 1, 2, 3, 5, 8, 13}}};
  std::string result = compat::format("{}", nested_map);
...
TEST(FormatterTest, EdgeCasesFormat) {
  ...
  EXPECT_EQ("[42]", compat::format("{}", single_vec));

  std::map<std::string, int> single_map = {{"key", 42}};
  EXPECT_EQ("{key: 42}", compat::format("{}", single_map));

  std::vector<std::vector<int>> nested_empty = {{}, {1, 2}, {}};
  EXPECT_EQ("[[], [1, 2], []]", compat::format("{}", nested_empty));
}
...
TEST(FormatterTest, SmartPointerFormat) {
  ...
  EXPECT_EQ("[42, 123, null]", compat::format("{}", int_ptrs));
  ...
  EXPECT_EQ("[hello, world, null]", compat::format("{}", str_ptrs));
  ...
  EXPECT_EQ("{null: null, one: 1, two: 2}", compat::format("{}", map_with_ptr_values));
  ...
  EXPECT_EQ("[{a: 1, b: 2}, {c: 3, d: null}]", compat::format("{}", nested));
}
...
TEST(FormatterTest, StatisticsFileFormat) {
  ...
  const std::string expected =
      ...
      "]"
      "]";
  EXPECT_EQ(expected, compat::format("{}", statistics_file));
}
